...
class Inverter(object):
    ...
    def __init__(self, ac_capacity, eff_ref=0.94, eff_nom=1.0):
        super().__init__()
    ...
    def ac_output(self, dc_in):
        ...
        if dc_in == 0 :
            return 0
        elif  math.isnan(dc_in):    #hier Nan für fehlende Daten
            return math.nan
        else:
...
def run_model(
    ...
    **kwargs,
    ...
    # Process data
    dir_horiz = data.global_horizontal
    diff_horiz = data.diffuse_fraction      #Changed from original for DWD-Data
    ...
    panel_efficiency = 0.20     #0.1
    area_per_capacity = 0.0048844       #0.001 / panel_efficiency

    panel = panel_class(
        panel_aperture=capacity * area_per_capacity,    #m2
    ...
    dc_out = pd.Series(output, index=datetimes).fillna(math.nan).clip(upper=capacity) #eddited for Nan
