...
def sun_angles(datetime_index, coords, rise_set_times=None):
    ...
    for index, item in enumerate(datetime_index):
        ...
        if rise_time is not None and item + datetime.timedelta(minutes=10)>=rise_time and \
                item<=rise_time + datetime.timedelta(minutes=10):
            # Special case for sunrise hour
            duration = 10 - (rise_time.minute - item.minute) - (rise_time.second / 60.0)
            obs.date = rise_time + datetime.timedelta(minutes=duration+15)
            sun_alt, sun_azimuth = _sun_alt_azim(sun, obs)
        elif set_time is not None and item + datetime.timedelta(minutes=21) >= set_time >= item:
            # Special case for sunset hour
            duration = set_time.minute - item.minute + set_time.second / 60.0
            obs.date = item - datetime.timedelta(minutes=15)
            sun_alt, sun_azimuth = _sun_alt_azim(sun, obs)
        else:
            # All other hours
            duration = 10
            obs.date = item + datetime.timedelta(minutes=5)
            sun_alt, sun_azimuth = _sun_alt_azim(sun, obs)
    ...
    #df.to_excel('Angles.xlsx', sheet_name='sheet0', index=True)
    return df
...
def aperture_irradiance(
    ...
    dni = (direct * (angles["duration"] / 10)) / np.cos(angles["sun_zenith"])
    #dni.to_csv("dni.csv")
    if dni_only:
        return dni
    ...
    plane_direct = (dni * np.cos(incidence)).clip(lower=0)          #Edited for Nans (siehe temp2.txt)
    plane_diffuse = (
        diffuse * ((1 + np.cos(panel_tilt)) / 2)
        + albedo * (direct + diffuse) * ((1 - np.cos(panel_tilt)) / 2)
    )
    df = pd.DataFrame({"direct": plane_direct, "diffuse": plane_diffuse})
    #df.to_excel('irradiance.xlsx', sheet_name='sheet0', index=True)
    return df
